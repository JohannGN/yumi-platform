...
// Restaurantes y riders para dropdowns de filtro
...
export default function AdminPedidosPage() {
  ...
  // Auto-refresh cada 30s para órdenes activas
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setRestaurants(
          (restRes.restaurants ?? []).map((r: { id: string; name: string }) => ({ id: r.id, name: r.name }))
        );
        setRiders(
          (riderRes.riders ?? []).map((r: { id: string; name: string }) => ({ id: r.id, name: r.name }))
        );
      } catch (e) {
        console.error(e);
      }
  ...
  // Auto-refresh cada 30s (pausar si hay panel abierto)
  useEffect(() => {
    if (selectedOrder) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Gestión de Pedidos</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {total > 0 ? `${total} pedido${total !== 1 ? 's' : ''} encontrado${total !== 1 ? 's' : ''}` : 'Vista de operaciones diarias'}
          </p>
        </div>
        <button
          onClick={() => fetchOrders()}
