...
{/* Footer */}
      <div className="p-4 border-t border-gray-200 dark:border-gray-800">
        <p className="text-[10px] text-gray-400 dark:text-gray-600 text-center">YUMI Panel v1.0</p>
      </div>
    </aside>
  );
}
...
function WhatsAppPill() {
  ...
  return (
    <button
      ...
      title={isActive ? 'WhatsApp activo' : 'Toca para activar WhatsApp'}
      className={`
        flex items-center gap-1.5 px-2.5 py-1 md:px-3 md:py-1.5 rounded-full text-xs font-semibold
        transition-all duration-200
${isActive
          ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400'
          : 'bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400'
    ...
    >
      <span className="relative flex-shrink-0">
        <span className={`block w-2 h-2 rounded-full ${isActive ? 'bg-green-500' : 'bg-red-500'}`} />
        <span className={`absolute inset-0 w-2 h-2 rounded-full ${isActive ? 'bg-green-400' : 'bg-red-400'} animate-ping`} />
      </span>
      ...
      {!isActive && <span className="hidden min-[360px]:inline">INICIO</span>}
    </button>
  );
}
// ─── WHATSAPP STATUS ────────────────────────────────────────
